...
5. 点击提交，完成设备配置；
6. 点开设备卡片中的工作状态开关，使设备进入**运行中**的状态。
...
1. `清空`按键，一键删除所有创建的组。当用户选择了一些组后，点击`删除`按键，可以快速批量删除选中的组。`创建`按键用于创建新的组。
2. 全选列表中的所有组。
3. 单选列表中的某个组。
4. `查看` 图标用于查看组的配置信息。
5. `创建` 图标用于为当前组创建 tags。
6. `删除` 图标用于删除当前组。
...
`导出`按键也可用于将已经创建的数据标签详细信息导出到 Excel 文件中。
`清空`按键用于一键删除列表中的所有数据标签。`删除`按键用于删除选定的标签。

### 第八步，设置数据标签详细信息

单击`创建`按键，进入数据标签的信息设置界面，如下图所示。在本例中，我们将介绍手动添加标签的方式。

![tags-add](./assets/tags-add.png)

将向您展示如何一个一个地手动添加标签设置信息。但是，所有的数据标签都可以先输入到 Excel 表中，然后通过再导入到组中。

为组创建标签：

1. 填写 Tag 名称，例如，tag1；
2. 填写驱动地址，例如，1!40001。详细的驱动地址使用说明请参阅 [驱动使用说明](../module-plugins/module-driver.md)；
3. 拉选择 Tag 类型，例如，Read， Write，Subscribe 暂时不支持 ；
4. 下拉选择数据类型，例如，int16；
5. 单击`添加`按键添加下一个标签，重复 1 - 4 知道添加到最后一个标签；
6. 可选择删除一个标签；
7. 点击`创建`按键，完成 Tag 的创建；
...
7. 选填设置的用户名；
8. 选填设置的密码；
9. 点击`提交`，完成北向应用的配置，点开应用卡片中的工作状态开关，使应用进入**运行中**的状态。

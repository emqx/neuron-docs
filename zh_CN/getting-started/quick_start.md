...
* 单击左上角的`添加设备`按键以显示添加新设备的对话框；
* 填写设备名称，例如 modbus-tcp-1；
* 点击下拉框，将显示现在可用的所有南向驱动协议，此次我们选择 modbus-tcp 的插件；
* 点击`创建`按键创建设备。
...
* 填写运行 Modbus 模拟器的 IP；
* 填写 Modbus 模拟器的端口号，默认是 502；
* 填写连接请求超时时间，默认是 3000；
* 点击提交，完成设备配置，设备卡片自动进入**运行中**的状态；

::: tip
运行的 Neuron 和模拟器必须要在同一个网段下。

不同的驱动，配置参数有所区别，详细的驱动配置参数说明可参考 [模块配置](../module-plugins/module-driver.md)。
:::
...
![group_list](./assets/group_list.png)

组列表中包含了以下信息：

* `导入`按键，以 Excel 形式，并以组为单位批量导入。`导出`按键，导出选中的组的信息到 Excel 中。`创建`按键用于创建新的组。`清空`按键，一键删除所有已创建的组。当用户选择了一些组后，点击`删除`按键，可以快速批量删除选中的组。
* 全选列表中的所有组。
* 单选列表中的某个组。
* `编辑` 图标用于修改组的配置信息。
* `Tag 列表` 图标用于为当前组创建 tags。
* `删除` 图标用于删除当前组。
...
* 填写 Tag 名称，例如，tag1；
* 下拉选择 Tag 属性；
* 下拉选择数据类型，例如，int16。由于每个驱动支持的类型不同，下拉框可选择的数据类型也就不同；
* 填写驱动地址，例如，1!40001。详细的驱动地址使用说明请参阅 [模块配置](../module-plugins/module-driver.md)；
* 乘系数，可选参数，可设置 Neuron 采集数值后的乘数，例如 neuron 采集到的数值是 11，Decimal 设置为 0.1，则该点位显示的数值为 1.1；
* 描述，可选参数，用于给标签添加一些描述信息；
* 单击`添加`按键添加下一个标签，重复 1-6 的步骤直到添加到最后一个标签；
* 新增一个标签后，信息框旁边会增加一个`删除`按键，可选择删除该标签；
* 点击`创建`按键，完成 Tag 的创建；

::: tip
目前，Tag 属性支持 Read, Write, Subscribe 三种，当选择 Subscribe 属性时，只有在采集到的数值发生变化时，才会将变化后的数值上传到云端。

当数据类型选择 float/double 时，会显示一个可选参数精度，用于设置精度，可选范围 1-17。
:::
...
* 下拉框选择想要查看的南向设备，这里选择上面已经创建好的 modbus-tcp-1;
* 下拉框选择想要查看所选南向设备下的组，这里选择上面已经创建好的 group-1；
...
* 应用名称 - 用户为应用程序设置的唯一名称。
* 应用配置按键 - 单击此按键以显示应用程序配置界面。
* 数据统计 - 统计应用节点卡片的信息。
* 更多
    * DEBUG 日志 - 打印节点 debug 日志，十分钟后恢复默认日志等级。
    * 删除 - 将此节点从北向应用列表中删除。
* 工作状态 - 显示应用节点的当前状态，五种工作状态。
...
* Client-id, 必填；
* Upload topic，必填，填写订阅数据上报的主题，详细订阅主题请参阅 [MQTT API](../reference/mqtt-api.md)；
* Heartbeat topic，必填，填写心跳报文上报的主题；
* Upload format，必填，选择上传格式；
* Cache memory size，必填，当 mqtt 连接异常时，设置上传数据缓存在内存中的大小；
* Cache disk size，必填，当 mqtt 连接异常时，设置上传数据缓存在磁盘中的大小；
* Host，必填，填写 MQTT Broker 的 hostname，默认连接的是 emqx 公共的 broker；
* Port，必填，填写 MQTT Broker 的端口号，默认是 1883；
* Username，选填，设置的用户名；
* Password，选填，设置的密码；
* 点击`提交`，完成北向应用的配置，应用卡片自动进入**运行中**的状态。
...
* 添加新的订阅，默认的上传 Topic 的主题格式为`/neuron/{node_name}/upload`，其中 {node_name} 是在 Neuron 界面中北向应用中配置的，本例我们填写上面设置好的 `/neuron/mqtt/upload`；

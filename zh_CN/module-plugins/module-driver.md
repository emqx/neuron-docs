...
本节主要介绍了北向应用和南向设备的参数配置和南向设备的点位信息配置规范。
...
| DB0~999.DBW   | int16/uint16/bit/int32/uint32/float/double/string | 读/写 | 全局数据库块，DB 是数据块，DBW是字偏移量，指数据块中起始数据字   |
...
当数据类型是 string 类型时，是必填项，**.LEN** 表示字符串长度，包含 **H** 和 **L** 两种字节顺序，不填默认是 **L** 字节顺序。
...
| 区域 |数据类型 | 属性  | 备注                           |
| ---- | --------- | ---------- | -------------------------------- |
| X    | 所有类型       | 读/写 | 输入继电器  (Q/iQ-F)             |
| DX   | 所有类型       | 读/写 | (Q/iQ-F)                         |
| Y    | 所有类型       | 读/写 | 输出继电器 (Q/iQ-F)            |
| DY   | 所有类型       | 读/写 | (Q/iQ-F)                         |
| B    | 所有类型       | 读/写 | 链接继电器 (Q/iQ-F)              |
| SB   | 所有类型       | 读/写 | 链接专用继电器               |
| M    | 所有类型       | 读/写 | 内部继电器 (Q/iQ-F)          |
| SM   | 所有类型       | 读/写 | 特殊寄存器 (Q/iQ-F)           |
| L    | 所有类型       | 读/写 | 锁存器 (Q/iQ-F)             |
| F    | 所有类型       | 读/写 | 信号器 (Q/iQ-F)             |
| V    | 所有类型       | 读/写 | 边缘继电器 (Q/iQ-F)              |
| S    | 所有类型       | 读/写 | (Q/iQ-F)                         |
| TS   | 所有类型       | 读/写 | 定时器触点 (Q/iQ-F)           |
| TC   | 所有类型       | 读/写 | 定时器线圈 (Q/iQ-F)              |
| SS   | 所有类型       | 读/写 | (Q/iQ-F)                         |
| STS  | 所有类型       | 读/写 | 保持定时器触点 (Q/iQ-F) |
| SC   | 所有类型       | 读/写 | (Q/iQ-F)                         |
| CS   | 所有类型       | 读/写 | 计数器触点 (Q/iQ-F)         |
| CC   | 所有类型       | 读/写 | 计数器线圈 (Q/iQ-F)            |
...
当数据类型是 string 类型时，是必填项，**.LEN** 表示的是字符串长度，包含 **H** 和 **L** 两种字节顺序，不填，默认的是 **L** 的字节顺序。
*例如*：

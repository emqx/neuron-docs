...
a1e 插件用于通过以太网访问三菱的A系列、FX3U、FX3G、iQ-F系列PLC。
...
| 区域 | 数据类型 | 属性  | 备注                                  |
| ---- | -------- | ----- | ------------------------------------- |
| X    | bit      | 读/写 | 输入继电器  (FX/iQ-F)                 |
| Y    | bit      | 读/写 | 输出继电器 (FX/iQ-F)                  |
| M    | bit      | 读/写 | 内部继电器 (FX/iQ-F)                  |
| L    | bit      | 读/写 | 锁存器 (FX/iQ-F)                      |
| F    | bit      | 读/写 | 信号器 (FX/iQ-F)                      |
| B    | bit      | 读/写 | 链接继电器 (FX/iQ-F)                  |
| SB   | bit      | 读/写 | 链接专用继电器(FX/iQ-F)               |
| S    | bit      | 读/写 | 步继电器(FX/iQ-F)                     |
| D    | 所有类型 | 读/写 | 数据寄存器 (FX/iQ-F)                  |
| W    | 所有类型 | 读/写 | 链接寄存器 (FX/iQ-F)                  |
| TS   | bit      | 读/写 | 定时器触点 (FX/iQ-F)                  |
| TC   | bit      | 读/写 | 定时器线圈 (FX/iQ-F)                  |
| TN   | 所有类型 | 读/写 | 定时器当前值 (FX/iQ-F)                |
| STS  | bit      | 读/写 | 保持定时器触点 (FX/iQ-F)              |
| STC  | bit      | 读/写 | 保持定时器线圈(FX/iQ-F)               |
| STN  | 所有类型 | 读/写 | 保持定时器 (FX/iQ-F)                  |
| CS   | bit      | 读/写 | 计数器触点 (FX/iQ-F)                  |
| CC   | bit      | 读/写 | 计数器线圈 (FX/iQ-F)                  |
| CN   | 所有类型 | 读/写 | 计数器当前值  (FX/iQ-F)               |
| LCS  | bit      | 读/写 | 长计数器触点（FX/iQ-F）               |
| LCC  | bit      | 读/写 | 长计数器线圈（FX/iQ-F）               |
| LCN  | 所有类型 | 读/写 | 长计数器当前值（FX/iQ-F）             |
| SB   | bit      | 读/写 | 链接特殊继电器（FX/iQ-F）             |
| SW   | 所有类型 | 读/写 | 链接专用寄存器 (FX/iQ-F)              |
| SM   | bit      | 读/写 | 特殊寄存器 (FX/iQ-F)                  |
| SD   | 所有类型 | 读/写 | 专用寄存器Specical register (FX/iQ-F) |
| Z    | 所有类型 | 读/写 | 索引寄存器 Index register (FX/iQ-F)   |
| LZ   | 所有类型 | 读/写 | 长变址寄存器 (FX/iQ-F)                |
| DX   | bit      | 读/写 | 链接直接软元件 链接输入(FX/iQ-F)      |
| DY   | bit      | 读/写 | 链接直接软元件 链接输出(FX/iQ-F)      |
| R    | 所有类型 | 读/写 | 文件寄存器 (FX/iQ-F)                  |
